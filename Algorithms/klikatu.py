...
def click_neg(event):
    ...
    plt.title(f"Puntu negatibo kopurura: {len(coordenadas_x_neg)}")
    plt.draw()
# Crear un gráfico vacío
fig, ax = plt.subplots()
...
w_hat,theta,w,x_berriak = soft_SVM_SGD(x_vectors,y_vectors,10000,0.1,standardize=True,plot=True)
b = w[0]
b2 = w_hat[-1,0]

...
plt.text(0, .15, r"$\mathcal{X} = [-5,5] \subset \BbbR$", fontsize=30, color='black')
plt.show()
...
plt.tick_params(axis='y', which='both', left=False, right=False)
plt.xlabel(r'$x$', fontsize = 25)
plt.ylabel(r'$x^2$', rotation = 0, fontsize = 25, labelpad= 8)
# l = plt.legend(["+","-"])
# l.get_frame().set_alpha(1) 
...
plt.xlim([-4,4])
plt.ylim([-2,16])
plt.text(1.5, 13.5, r"$\hat{\mathcal{X}} \subset \BbbR^2$", fontsize=30, color='black')
plt.tight_layout(pad = 0.2)
plt.show()
